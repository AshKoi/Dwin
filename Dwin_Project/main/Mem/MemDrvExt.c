...
* 输　出:
* 返  回:
* 功  能: 交采冻结数据处理 2024.7.22
*********************************************************************************/
INT32S  MemWriteAcDJHistDataProc(INT32U nFn, INT16U nPreNum, void *pBuf, INT32U nLen, INT8U nUpdateFlg)
{
    //INT16U      nPre = 0;
    INT16U      nIdx = 0;
    ...
    //1.获取时间存储信息
    if (RET_ERR_OK != MemFindACDJHistDataIdx(nFn, nPreNum, &nIdx))
    {
        return -1;
    }
    //2.存储
    ...
    if (nUpdateFlg)
    {
        MemUpdateHistDJDataMemInfo(nFn);
    }
...
INT32S  MemReadAcHistDataProtProc(INT32U nFn, INT16U nPreNum, void *pBuf, INT32U nLen)
{
    ...
    //GetPtrDay(&sTime, &sTmpTime, nDays);
    
    MemReadAcHistDataProc(nFn, nPreNum, &sAcDayData,sizeof(MEM_AC_DAY_DATA));
    memcpy(pBuf, &sAcDayData, sizeof(MEM_AC_DAY_DATA));
    
    /*for (j = 0; j < MAX_AC_DAY_DATA_NUM; j++)
    {
        memset(&sAcDayData, 0, sizeof(sAcDayData));
        MemReadAcHistDataProc(nFn, j, &sAcDayData,sizeof(MEM_AC_DAY_DATA));
        if (0 == memcmp(&sAcDayData.sTime.nDay, &sTmpTime.nDay, sizeof(FORMAT20)))
        {
            memcpy(pBuf, &sAcDayData, sizeof(MEM_AC_DAY_DATA));
            break;
        }
...
* 输　出:
* 返  回:
* 功  能: 查找AC历史数据时标(待重新，获取AC_DAY_MEM_INFO错误,理乱上应该是sMemInfo.nMemNum而不是MAX_AC_MON_DATA_NUM)
*********************************************************************************/
INT8S MemFindACDJHistDataIdx(INT8U Fn, INT16U nPreNum, INT16U *pIdx)
{
    ...
    memset(&sMemInfo, 0, sizeof(sMemInfo));
    MemReadAcXFlhBackData(Fn, &sMemInfo, sizeof(AC_DAY_MEM_INFO));
    if (nPreNum > GET_MIN(sMemInfo.nMemNum, MAX_AC_MON_DATA_NUM))
    {
        return RET_ERR_RANG;
    }
    nIdx = (sMemInfo.nNext + MAX_AC_MON_DATA_NUM - nPreNum) % MAX_AC_MON_DATA_NUM;
	*pIdx = nIdx;
    return RET_ERR_OK;
}
...
INT8S MemUpdateHistDJDataMemInfo(INT8U Fn)
{
    ...
    memset(&sMemInfo, 0, sizeof(sMemInfo));
    MemReadAcXFlhBackData(Fn, &sMemInfo, sizeof(AC_DAY_MEM_INFO));
    sMemInfo.nNext = (sMemInfo.nNext + 1) % MAX_AC_MON_DATA_NUM;
    (sMemInfo.nMemNum < MAX_AC_MON_DATA_NUM) ? (sMemInfo.nMemNum++) : (sMemInfo.nMemNum = MAX_AC_MON_DATA_NUM);
    MemWriteAcXFlhBackData(Fn, &sMemInfo, sizeof(AC_DAY_MEM_INFO));
    return RET_ERR_OK;
}
